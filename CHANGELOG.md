...
- feat(ui): filter guests by name or ID from the panel and show a fade when more content is below the scroll area
- feat(sort): add a "Custom (drag to reorder)" sort mode. Guest order is saved per cluster and survives node migration. Dragging near a panel edge scrolls within the guest's section. Reordering is disabled while filtering
- feat(stats): explain missing guest IP addresses, including stopped guests, unavailable guest agents, permission errors, and interfaces with no IPv4 address
- fix(vnc): keep consoles connected when QEMU changes screen size
- refactor(console): share WebSocket URL construction and add tests for VNC button mapping and LXC authentication framing
- ci: build the plugin and run its test suite in Forgejo Actions
