...
Rectangle {
    ...
    RowLayout {
        ...
        Item {
            ...
            // all VM rows regardless of running state. Width tracks which
            // features are enabled: stats needs 1 slot, power actions up to 2
            // (shutdown + reboot). Each slot is a button plus its spacing, so
            // the reservation shrinks when a feature is toggled off instead of
            // leaving dead space. The actual buttons live in an inner RowLayout
